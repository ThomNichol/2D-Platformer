...
func _physics_process(_delta):
	...
	if $AnimatedSprite2D.animation == "NightBorneAttack": $AnimatedSprite2D.offset.x = 7*direction
...
func damage():
	if SM.state_name != "NightBorneDies":
		SM.set_state("NightBorneDies")
...
func _on_AnimatedSprite_animation_finished():
	if SM.state_name == "NightBorneAttack":
		SM.set_state("NightBorneMove")
	if SM.state_name == "NightBorneDies":
		queue_free()
